...
def test_model(model, train_loader, test_loader, device, args, ewc_loss):
    epoch = args.epochs
    model = load_model("./result/cifar10_oe/model", model, epoch)
    model.eval()
...
def get_score(model, device, train_loader, test_loader, epoch=0):
    ...
    threshold = mean_distance + 0.2 * stddev_distance
    ...
    with open("./result/" + args.save_name + "/distances.txt", "w") as f:
...
def main(args):
    ...
    test_model(model, train_loader, test_loader, device, args, ewc_loss)

...
def evaluation(model_name, encoder, bn, decoder, data_path, dist = 'L2', size = 32, dataset = 'cifar10', device = 'cpu'):
    ...
    threshold = np.max(train_maps) + 3 * std
    print(np.mean(train_maps))
    print(np.mean(pr_list[gt_list == 1]))
    print(np.mean(pr_list[gt_list == 0]))
    ...
    detect_rate = np.sum(pred_labels[gt_list == 1]) / len(gt_list[gt_list == 1])
    ...
    return auroc, detect_rate, total_acc
...
if __name__ == '__main__':
    ...
    auroc, detect_rate, total_acc = evaluation(args.model, encoder, bn, decoder, data_path, args.dist, args.size, args.dataset, device)
    print(f'AUROC: {auroc}')

...
class BN_layer(nn.Module):
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        return output.contiguous(), x.contiguous()
...
class ClassifierHead(nn.Module):
    def __init__(self, in_features, ratio = 0.25):
        super().__init__()
        self.classifier_head = nn.Sequential(
            nn.Linear(in_features=in_features, out_features = int(in_features*ratio)),
            nn.ReLU(inplace=True),
            nn.Linear(in_features=int(in_features * ratio), out_features = 1)
        )
